...
function nounUrl(minCorpusCount, limit) {
  return (
    'http://api.wordnik.com/v4/words.json/randomWords?hasDictionaryDef=false&includePartOfSpeech=noun&minCorpusCount=' +
    minCorpusCount +
    '&maxCorpusCount=-1&minDictionaryCount=1&maxDictionaryCount=-1&minLength=5&maxLength=-1&limit=' +
    limit +
    '&api_key=' +
    WordnikAPIKey
  )
...
function tweet() {
  ...
  if (debug) console.log('Debug mode: ', tweetText)
  else
...
function followAMentioner() {
  T.get('statuses/mentions_timeline', { count: 50, include_rts: 1 }, function(
    err,
    reply
  ) {
    if (err !== null) {
...
function respondToMention() {
  T.get('statuses/mentions_timeline', { count: 100, include_rts: 0 }, function(
    err,
    reply
  ) {
    if (err !== null) {
      ...
      if (debug) console.log(tweet)
        ...
        T.post(
          'statuses/update',
          { status: tweet, in_reply_to_status_id: mentionId },
          function(err, reply) {
            if (err !== null) {
              console.log('Error: ', err)
            } else {
...
function runBot() {
  ...
  request(nounUrl(5000, 200), function(err, response, data) {
    ...
    pre = [
      "Dude, I haven't even started this essay on " +
        pluralize(nouns.pick().word) +
        ' yet.',
      "I don't know how anybody can tolerate Prof. " +
        capitalize(singularize(nouns.pick().word)) +
        '. What a tool.',
      "I'm so behind in my " + singularize(nouns.pick().word) + ' class.',
      "I'm thinking of changing my major to " +
        capitalize(singularize(nouns.pick().word)) +
        ' Studies.',
      'Seriously, ' +
        capitalize(singularize(nouns.pick().word)) +
        ' Engineering is ruining my life.',
      "I can't believe I forgot to bring my " +
        nouns.pick().word +
        ' to lab again.',
      'Sooo much homework in this ' +
        capitalize(nouns.pick().word) +
        ' class. I should have taken ' +
        capitalize(nouns.pick().word) +
        ' instead.',
      'Almost the weekend! Totally amped for the ' +
        nouns.pick().word +
        ' party.',
      'Seriously I have had enough of Intro to ' +
        capitalize(nouns.pick().word) +
        '.',
      "Who's coming to Club " +
        capitalize(singularize(nouns.pick().word)) +
        " tonight? I'm DJing along with my bro " +
        capitalize(nouns.pick().word) +
        '.',
      'Missed class again. Too many ' +
        pluralize(nouns.pick().word) +
        ' last night.',
      // etc.
    ]
